// MainDashboard.js — 首頁元件 (UI.pen NEW-02)
const MainDashboard = {
    template: `
    ...
    setup() {
        ...
        const semesterInfo = Vue.computed(() => {
        ...
        const recentCourses = Vue.computed(() => {
            ...
            return Array.from(groups.values()).slice(0, 3).map(c => {
                ...
                if (c._periods.length > 0) {
                    const byDay = {};
                    c._periods.forEach(p => { (byDay[p.day] = byDay[p.day] || []).push(p.period); });
        ...
        return { displayName, isLoggedIn, semesterInfo, recentCourses, courseColors,
                 goToSchedule, goToQuery, goToBot, goToSettings };
    }
};
