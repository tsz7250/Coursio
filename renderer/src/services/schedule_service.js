...
class ScheduleService {
    ...
    getCourseSchedule(year, smtr) {
        ...
        if (!this.backend.ALLDATA["original_account"] || !this.backend.ALLDATA["original_password"]) {
            console.error("❌ 缺少登入憑證");
            this.setEmptyPersonalSchedule("缺少登入憑證");
            return Promise.resolve(this.backend);
        }

        return this.backend.puppeteerService.getCompleteScheduleData(year, smtr)
            .then((result) => {
                if (result.success) {
            ...
            .catch((error) => {
                if (error.message && error.message.includes('Target closed') && this.backend.course_schedule_data) {
